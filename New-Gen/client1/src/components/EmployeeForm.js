...
function EmployeeForm({ onFinish, initialValues }) {
  return (
    ...
    Birth" name="dateOfBirth" rules={[{ required: true }]}>
      ...
      <Row>
        <Col span={8} xs={24} sm={24} lg={8}>
          <Form.Item required label="Job Role" name="jobRole" rules={[{ required: true }]}>
            <Input placeholder="pc or laptop repair" />
          </Form.Item>
        </Col>
      </Row>
      <Row>
        <Col span={8} xs={24} sm={24} lg={8}>
          <Form.Item required label="Work Experience" name="workExperience" rules={[{ required: true }]}>
